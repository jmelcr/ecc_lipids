...
with negatively charged lipids \cite{??}.
...
concentration in the vicinity of the membrane \cite{??}. On the other hand,
PS headgroups can form dehydrated complexes with calcium \cite{??} and ?? are
proposed to form complexes with ions \cite{??}. Different scenarios are
difficult to evaluate experimentally, especially with physiological
ion concentrations which are typically quite low.

Spectroscopic experiments give detailed information about the
interactions \cite{??}. The data is, however, often indirect and difficult
to interpret. Classical molecular dynamics (MD) simulations give
access to all the details, but the current force fields are not
sufficiently accurate to interpret the interactions of cationic
lipid headgroups with other lipids and ions \cite{NMRlipidsIV}.
...
to anionic PS lipids to more accurately describe its interactions with ions
and PC headgroups in mixed lipid bilayer. The results pave
...
\todo{SAMULI: I would put under under this section.
1) a very short explanation of the usage of electrometer concept in this paper by mainly citing NMRlipids IV.
2) a very short explanation of the usage of form factor and order parameters by citing approriate publications}
...
\todo{SAMULI: Table of simulated systems would be useful.}
General simulation setup.
 

Any specifics for simulations with aqueous cations.
...
\todo{Combine with Fig.~\ref{fig:delta_ordPar_NaCl_PC-PS_mix} into a single plot.} \\
\todo{SAMULI: I do not see any reason why we should show this data in this paper at all.}
\end{figure} 
...
\todo{Josef: Simulation at 50:50 PC:PS might be useful for this plot (PC-PS interactions). 
Now it may make an impression, that we are consciously avoiding it. }
\end{figure} 
...
\todo{JM: Compare with pure POPC bilayer? 
SAMULI: Comparison of the relative surface excess values in the text and in the table would be probably sufficient.}
\end{figure} 
...
\todo{Look up possible applications of AMOEBA and Drude on anions/cations and possibly lipids. 
Does ECC provide improvment over them? 
This is crucial -- if this section will make the core message of this paper, 
I have to put the ECC-method into context of other polarizable models. 
If there are no other simulations like these and no data on the accuracy with cation/anions, 
our approach (ECC) is hence the pioneer in the field and, hence, sets the standard instead. 
SAMULI: We should keep the focus on lipids here. We should check the recent literature, but
I am not aware of charged lipid simulations with explicitly polarizable force fields. 
As for PC, the charge scaling was used in the early simulations of PS lipids from Berendsen group.
We should mention this. Otherwise, I think that one of our conclusions will be that our
results indicate that ECC generally improves the results of lipids bilayer MD simulations.
However, we should not write a separate section about it. It should be in the conclusions section.}
