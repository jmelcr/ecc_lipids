...
of phosphatidylcholine (PC) lipids \cite{catte16, nmrlipids_proj4}, which can
...
mixtures with negatively charge PS lipids \cite{nmrlipids_proj4}.
...
The quality of the newly developed ECC-POPS force field parameters is
evaluated against previously published X-ray scattering form factors\cite{kucerka14}
and NMR \cite{roux90,nmrlipids_proj4} data for lipid headgroup C--H bond order parameters in various ionic conditions,
as well as against acyl chain C--H bond order parameters reported in this work.
as well as 
...
the default Amber ion parameters \cite{aqvist90} led to the artificial clustering of ions in solution \cite{nmrlipids_proj4}.
...
error estimate \cite{botan15,ollila16,nmrlipids_proj4}.
...
As discussed in our previous work \cite{nmrlipids_proj4}, the hydration levels of
...
the headgroup and glycerol backbone region of POPS lipid bilayer are already available in the literature \cite{kucerka14,nmrlipids_proj4}.
...
The value of $\Gamma ^{w} _{\rm Na}=-0.11 \pm 0.01$ nm$^{-2}$ for 1 M sodium in pure POPC
was reported from a ECC-POPC simulation in the previous work \cite{melcr18}.
...
response to NaCl is also slightly overestimated \cite{melcr18}.
