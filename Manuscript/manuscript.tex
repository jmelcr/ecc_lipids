...
mixtures\cite{roux90,NMRlipidsIV}. The evaluation of monovalent ion binding affinity to
lipid bilayers containing PS lipids is complicated also by the lack of ion-free state due to the 
counterions, which are always present with negatively charged lipids~\cite{NMRlipidsIV}.
Therefore, we evaluate the counterion binding affinity to POPC:POPS mixtures by monitoring
the response of POPC headgroup order parameters to both increasing mole fraction of PS lipids and
...
However, in simulations with strong counterion binding affinity, the charge of bound
counterions cancel the influence of negatively charged lipids and the increase in PC headgroup
order parameters is not observed \cite{NMRlipidsIV}. This is observed in 
Lipid17/Dang ion simulations with \ce{Na+} counterions (Fig~\ref{fig:delta_ordPar_NaCl_PC-PS_mix_PC}),
while in ECC-lipids simulations with \ce{Na+} counterions and in Lipid17/Dang
%The trend is observed with both force fields in simulations
with \ce{K+} counterions the order parameters increase
in 
...
This could be explained by the deeper penetration of \ce{K+} into the bilayer (Fig.~\ref{fig:delta_ordPar_NaCl_PC-PS_mix_PC}).

Because experimental data with the additional amount of \ce{Na+} in POPC:POPS (5:1) mixture was not available,
we compared the simulation results to the experimental data with the additional amounts of
\ce{Li+} and \ce{K+} ions, which should give the lower and the upper bounds to the sodium binding
...
%are clearly overestimated in Lipid17/Dang simulations and slightly overestimated in ECC-lipids simulations,
...
are too sensitive to an additional amount of \ce{KCl} and \ce{NaCl} 
...
to $\Gamma^{w}_{Ca} = 0.24\mathrm{nm^{-2}}$ ($\sim 4.8$ phospholipids per bound \ce{Ca^{2+}}) for the POPC:POPS (5:1) bilayer with 409 mM CaCl$_2$.
...
\todo{J: I have moved several paragraphs from the body to here and added a bit of glue. This section still requires to be written properly.  }
