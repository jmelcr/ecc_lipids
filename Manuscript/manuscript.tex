...
\todo{JOE: split this figure into two: separate for POPS (to SI) and PC:PS 5:1 (main text)? SAMULI: Should be POPS in main text, mixture in SI.}
\end{figure} 
...
First, we monitor the response of PC headgroup order parameters to the increasing amount of PS (Fig.~\ref{fig:delta_ordPar_NaCl_PC-PS_mix}).
According to the electrometer concept, the head group order parameters of PC lipids increase
with the increasing amount of negatively charged PS lipids in experiments \cite{seelig87,scherer87}.
...
in both force fields (Fig. \ref{fig:delta_ordPar_KCl_PCPS}), athought the change of the $\alpha$-carbon
...
affinity (Fig. \ref{fig:delta_ordPar_KCl_PCPS}). The PC headgroup response ot the additional
amount of sodium is clearly overestimated by the Lipid17/Dang simulation, which is in line
the higher binding affinity (Fig. \ref{fig:POPS-counterions-dens}). Also, the response
in ECC-POPS simulations is probably too close to the experimental results for lithium,
indicating slight overbinding also in this simulation. In conclusion, the response of PC
...
the binding affinity. The binding affinity of potassium is better described by both force fields.
