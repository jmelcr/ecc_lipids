...
\end{equation}
The value of $\epsilon_{el}$ for water solutions is $\epsilon_{el} = 1.78$ \cite{leontyev09},
hence, the scaling factor for aquaeous environments yields $f_q \approx 0.75$. 
...
In the case of POPS, which is a monovalently negatively charged lipid, 
...
Similarly to the ECC-POPC model, we changed the charges only of the polar parts of the phospholipid,
i.e., charges on all atoms except on alkyl tails, 
which do not encounter direct contacts with ions. 

Note that we used a different scaling factor $f_q = 0.8$ 
to for the ECC-POPC model. \cite{melcr18}
...
We used the SPC/E~\cite{Berendsen1987} water model \todo{SAMULI: The choice of water model should be justified.}
...
The ECC-POPS model as derived again from the Lipid14
\todo{SAMULI: References to Lipid14/Lipid17 models should be consistent here and elsewhere.
We could also consider using term Amber lipd force field.}
force field~\cite{dickson14, lipid17-future}, as was described above.
For a direct comparison, 
we also performed simulations with the Lipid14
\todo{SAMULI: Also Lipid17 is used. The origin of Gromacs suitable files (Zenodo, acpype, etc.) should be described.}
model~\cite{dickson14},
...
\begin{figure}[tbp!] 
  \centering 
  \includegraphics[height=\figheightsmall]{../img/ecc_pops/density_profiles_na-k-counterions_wat_phos_compar_purePOPS.pdf}
  \includegraphics[height=\figheightsmall]{../img/ecc_pops/density_profiles_na-k-counterions_wat_phos_compar_5PC-1PS.pdf} \\
  \includegraphics[height=\figheightsmall]{../img/ecc_pops/l17/density_profiles_na-k-counterions_wat_phos_compar_purePOPS_lipid17.pdf}
  \includegraphics[height=\figheightsmall]{../img/ecc_pops/l17/density_profiles_na-k-counterions_wat_phos_compar_5PC-1PS_lipid17.pdf}
  \caption{\label{fig:POPS-counterions-dens}
    ...
    using ECC-lipids and ECC-ions (top) and Lipid17 (bottom).  }
\end{figure} 
