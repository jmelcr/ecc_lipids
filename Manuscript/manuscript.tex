...
\begin{abstract} 
  ...
  \todo{The way we write "head groups" resp. "headgroups" has to be united. 
        I prefer "head groups" as it is gramatically correct, but I am fine with either of the options.}
...
As such \todo{lipid-related functions resp. signalling (choose more suitable)} are also often regulated by ions \cite{leventis10},
...
Spectroscopic experiments give \todo{detailed .vs. accurate (better?). 
Reason: I did not like that the sentence says that experiments give detailed information and at the same time such information is indirect and difficult to interpret.} 
...
\todo{The given range is not correct, e.g. scaling of POPC (0.89), Mg2+ (0.72 and 0.82). 
Extending the lower bound to 0.8 fits the cases I know of with the exception of the small-Mg2+. }
...
% above does not apply to molecules and moieties with non-zero total charge (hence the edit). 
...
only in the headgroup, glycerol backbone, and carbonyl regions. % added because the information where we scale charges was missing before.
...
only in the headgroup, glycerol backbone, and carbonyl regions \cite{melcr18}. % Can be shortened by saying it is the same set as that for scaling charges.
...
\begin{table*}[tbp]
\centering
\caption{  MOVE THE WHOLE TABLE TO SI!!! % J: I do not have a strong opinion on this. I think it can stay within the main text in a JCTC paper.
...
\begin{table}[tbp]
  \caption{MOVE TABLE TO SI AND MENTION ONLY THE KEY INFO IN 1 SENTENCE IN THE MAIN TEXT  % J: I am fine with this, the simulation parameters are rather standard. There is nothing to be done in the text, the reference to this table will only obtain prefix "S".
...
\begin{figure}[tbp!] 
  \centering 
  \includegraphics[width=\figwidthsmall]{../img/ecc_pops/density_profiles_na-k-counterions_wat_phos_compar_purePOPS_ecclipids-lipid17.pdf}
...
\begin{figure}[tbp!] 
  ...
  \caption{\label{fig:delta_ordPar_monoval_PCPS}
    ...
    Inset in A shows the labeling of carbon segments in a POPC head group.}
\end{figure} 
...
\begin{figure*}[!tbp] 
  ...
  \caption{\label{fig:delta_ordPar_NaCl_PC-PS_mix_PC} 
    ...
    (only \ce{Na^+} counterions, but shown also in the left plots (B) for \ce{K+}).
...
%
% J: I deemed the following sentences need improvement, my suggestion is below.
%
...
Slightly overestimated binding of \ce{Na+} to a POPC bilayer can be deduced from
the response of headgroup order parameters to the addition of NaCl (Fig~3 in Ref.~\citenum{melcr18}) 
and similarly also that to a POPC:POPS (5:1) mixture from the discussion above. 
In spite of this small inaccuracy
the increase in binding affinity of \ce{Na+} due to PS lipids remains unaffected.
...
in line with increased relaxation times of fluorescence probes from experiments. \cite{melcrova16}
...
%  \end{tabular}
  \caption{
    ...
    along the normal of the negatively charged membrane with the composition 5\,PC:1\,PS
    at 80~mM (A, top) and 200~mM (A, bottom) added buffer concentrations of \ce{CaCl2} from simulations with ECC-lipids (solid) and Lipid17 (dashed). 
    ...
    as a function of \ce{CaCl$_2$} added buffer concentration from ECC-lipids and Lipid17/Dang simulations 
...
\begin{figure}[tbp!] 
  ...
  \caption{ \label{fig:cacl_complexes} 
    ...
    in exclusive contact with the given oxygen moiety
    when bound to PC only, PS only or simultaneously to both
    calculated from ECC-lipids simulation of POPC:POPS (5:1) bilayer with 409 mM CaCl$_2$ added buffer concentration. 
	In the case of simultaneous binding to both PC and PS,
	the percentages refer to the moieties in PS.
        For numerical values, see table~\ref{tab:binding}.
    \textbf{(B)} Relative probabilities of \ce{Ca^{2+}} ions to coordinate with a certain number of lipids
    in pure POPC bilayer with 350 mM CaCl$_2$ and in POPC:POPS (5:1) mixture with 400 mM CaCl$_2$.  
    Analysis was done for both systems by considering all lipids (blue and violet) and
...
\todo{MOVE THIS WHOLE PARAGRAPH TO SI}
...
due to the attraction between \ce{COO^-} groups in PS and the cations adsorbed at the phosphate region.
...
and demonstrate the usefulness of ECC also for charged lipids. The general philosophy behind our approach is to build upon existing non-polarizable force fields making the minimum of necessary changes by scaling only groups bearing a sizeable (partial) charge, followed by slight readjustments of van der Waals radii and possibly also other parameters such as those concerning dihedral angle terms. The present work thus represents another piece in the mosaic that leads to development of an accurate empirical force field for biomolecular simulations involving aqueous salt solutions, proteins, peptides, lipid membranes, and/or nucleic acids, which include electronic polarization effects in a mean field way via charge scaling without extra computational costs compared to standard non-polarizable simulations. 
