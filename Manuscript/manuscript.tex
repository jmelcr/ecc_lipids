...
In this method, the effects of electronic polarization are represented through rescaling of charges. \cite{melcr18}
The scaling factor is derived from the high-frequency dielectric constant of electrons, $\epsilon_{el}$, as
\begin{equation}
f_q = \frac{1}{\sqrt{\epsilon_{el}}} .
\end{equation}
The value of $\epsilon_{el}$ for water solutions is $\epsilon_{el} = 1.78$ \cite{leontyev09},
hence, the scaling factor yields $f_q \approx 0.75$. 

In the case of POPS, which is a monovalently negatively charged lipid, 
we used exactly this scaling factor to represent the polarizability of the electrons
yielding a total charge of $-0.75$ in the ECC formalism. \cite{martinek17, leontyev09, melcr18}

Note that we used a different scaling factor of $f_q = 0.8$ 
to derive a ECC-POPC model, which also uses ECC. \cite{melcr18}
...
Partial charges of the atoms in POPC are not physical observabels, 
...
was determined as less polar compared to other models. 

\textbf{This explains the choice of different scaling factor. uff..}
