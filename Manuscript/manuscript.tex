...
order parameters also from the acyl chain region using R-PDLF experiments as described previously \cite{ferreira13}.
...
To evaluate the effect of ECC on the structural properties of POPS lipid bilayer properties in MD simulations,
we compared the results from a simulation with original Lipid17 \cite{??} combined with the Dang ion parameters \cite{??}
with the results from ECC-POPS simulation and experiments (Fig. \ref{simVSexpNOions_POPS}).
with sodium counterions are prevously publihed
(or only counter ions)
